...
$(document).ready(function () {
    ...
    initializeGridCheckboxes('SvyazCallRequestsGrid');
...
function initializeControlButtons() {
    ...
    $("#newrequest").click(function() {
        ...
        window.open("/SvyazCallRequest/Index", "_blank");
    ...
    $("#advsearch").click(function () {
        ...
        $.ajax({
            type: 'Post',
            cache: false,
            url: '/SvyazCallRequestList/GetSvyazThemes',
            dataType: 'json',
    ...
    $("#deleterequest").click(function () {
        ...
        var selected = $("#SvyazCallRequestsGrid [name=id]:checked").map(function () {
        ...
        if (checkGridSelection === "Selected") {
            ...
            if (r === true) {
                ...
                $.ajax({
                    type: 'Post',
                    cache: false,
                    url: '/SvyazCallRequestList/DeleteCallRequests/',
    ...
    for (var i = 0; i < people.length; i++) {
        $(people[i]).autocomplete({
            source: function(request, response) {
                $.ajax({
                    url: "/SvyazCallRequestList/GetPeople",
                    type: "POST",
                    dataType: "json",
